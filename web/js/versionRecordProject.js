$(document).ready(function () {
    ...
    //列表按钮事件绑定
    $(document).on("click",".btn-see-version",function(){
        var colDbId = $(this).parent().parent().find(".db-id");
        var name = $(this).parent().parent().find(".name").text();
        localStorage.setItem("versionProjectName", name);
        localStorage.setItem("versionProjectId", colDbId.text());
        location.href ="versionList";
    });
    //编辑按钮
    $(document).on("click", ".btn-edit-project", function () {
        var colDbId = $(this).parent().parent().find(".db-id");
        localStorage.setItem("edit-version-project-id", colDbId.text());
        var name = $(this).parent().parent().find(".name").text();
    ...
    $("#btn-edit-save").click(function () {
        var projectId = localStorage.getItem("edit-version-project-id");
...
function getProjectList(userId, index) {
    ...
    $.get("/ZzApiDoc/v1/versionProject/getAllVersionProjectWeb?userId=" + userId + "&page=" + index,
        function (data, status) {
            if (status === 'success') {
                //填充表格
                var c = "";
                $.each(data.rows, function (n, value) {
                    c += '<tr>' +
                        ...
                        '<td><button type="button" class="btn-see-version btn">版本管理</button></td>' +
...
function doDelete() {
    ...
    $.post("/ZzApiDoc/v1/versionProject/deleteVersionProjectWeb", {
            userId: userId,
