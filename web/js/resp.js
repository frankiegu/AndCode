$(document).ready(function () {
    ...
    /*返回参数-父参数*/
    $(document).on("click", ".btn-see-p-res", function () {
        var colDbId = $(this).parent().parent().find(".db-pid");
        getParentPid(colDbId.text());
    });
    /*编辑按钮*/
                ...
                if (data.code === 0) {
                    ...
                    $.each(data.data, function (n, value) {
                        c += '<tr>' +
                            ...
                            '<td class="db-pid hide">' + value.pid + '</td>' +
                            ...
                            '<td><button type="button" class="btn-see-p-res btn' + ((value.pid === null || value.pid === '0') ? ' hide' : '') + '">父参数</button></td>' +
...
function getParentPid(pid) {
    var userId = localStorage.getItem("userId");
    $.get("/ZzApiDoc/v1/responseArg/getResponseArgDetails", {
            userId: userId,
            id: pid
        },
        function (data, status) {
            if (status === 'success') {
                if (data.code === 0) {
                    ...
                    // showOkMsg(data.msg);
                    localStorage.setItem("pid", (data.data.pid === null?'0':data.data.pid));
                    window.location.reload();
                }
            }
        });
