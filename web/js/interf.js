$(document).ready(function () {
    ...
    var pageIndex = localStorage.getItem("interfacePageIndex");
    ...
    if (userId === null || userId === "") {
        ...
        getProjectList(groupId, userId, pageIndex === null ? 1 : parseInt(pageIndex));
    }
    //请求参数
    ...
    $("#btn-refresh").click(function () {
        var userId = localStorage.getItem("userId");
        var groupId = localStorage.getItem("groupId");
        var index = localStorage.getItem("interfacePageIndex");
        getProjectList(groupId, userId, index === null ? 1 : parseInt(index));
    });
    //删除按钮点击
...
function getProjectList(groupId, userId, index) {
    ...
    if (search === null || search.length === 0) {
        ...
        url += ("&search=" + escape(escape(search)));
    }
...
function justUpdateList(groupId, userId, index) {
    ...
    //保存页码
    localStorage.setItem("interfacePageIndex", index);
    var url = "/ZzApiDoc/v1/interface/getInterfaceByGroupIdWeb?groupId=" + groupId + "&userId=" + userId + "&page=" + index;
    var search = $("#et-search").val();
    if (search === null || search.length === 0) {
    } else {
        url.append("&search=" + escape(escape(search)));
    }
    $.get(url,
        function (data, status) {
            if (status === 'success') {
