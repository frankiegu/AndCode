$(document).ready(function () {
    ...
    //编辑按钮
    $(document).on("click", ".btn-edit-qrcode", function () {
        var colDbId = $(this).parent().parent().find(".db-id");
        localStorage.setItem("edit-qrcode-id", colDbId.text());
        var name = $(this).parent().parent().find(".title").text();
        var note = $(this).parent().parent().find(".content").text();
        var type = $(this).parent().parent().find(".int-type").text();
        $("#select-qrcode-type-edit").selectpicker('val', type);
        $("#et-qrcode-name-edit").val(name);
        $("#et-note-edit").val(note);
    });

    //复制功能
    var clipboard = new ClipboardJS('.btn-copy-qrcode', {
    ...
    //查看二维码
    $(document).on("click", ".btn-see-qrcode", function () {
        var url = $(this).parent().parent().find(".img-url").text();
        $("#iv-qrcode").attr("src", url);
    });
    //登录按钮点击
    ...
    $("#btn-add-qrcode").click(function () {
        $("#select-qrcode-type").selectpicker('val', '0');
        $("#et-qrcode-name").val("");
        $("#et-note").val("");
    });
    ...
    $("#btn-edit-save").click(function () {
        var qrcodeId = localStorage.getItem("edit-qrcode-id");
        editQrcode(qrcodeId);
    });
...
function getQrcodeList(userId, index) {
    ...
    $.get("/ZzApiDoc/v1/qrcode/getAllQrCodeWeb?userId=" + userId + "&page=" + index,
        function (data, status) {
            if (status === 'success') {
                //填充表格
                var c = "";
                $.each(data.rows, function (n, value) {
                    c += '<tr>' +
                        ...
                        '<td class="int-type hide">' + (value.isPrivate === 'false' ? '0' : '1') + '</td>' +
                        '<td class="img-url hide">' + value.urls + '</td>' +
                        '<td class="title">' + value.title + '</td>' +
                        '<td class="content">' + value.content + '</td>' +
                        '<td>' + (value.isPrivate === "false" ? '公有' : '私有') + '</td>' +
                        '<td>' + value.createUserName + '</td>' +
                        '<td>' + value.createTime + '</td>' +
                        '<td><button type="button" class="btn-edit-qrcode btn btn-primary"  data-toggle="modal" data-target="#editModel">' +
                        '<span class="glyphicon glyphicon-pencil" aria-hidden="true"></span> 编辑</button></td>' +
                        '<td><button type="button" class="btn-copy-qrcode btn btn-primary">' +
                        '<span class="glyphicon glyphicon-copy" aria-hidden="true"></span> 复制</button></td>' +
                        '<td><button type="button" class="btn-see-qrcode btn btn-primary"  data-toggle="modal" data-target="#seeQrcodeModal">' + ' 查看二维码</button></td>' +
                        '</tr>';
                });
                $("#qrcode-list").html(c);
...
function doDelete() {
    ...
    $.post("/ZzApiDoc/v1/qrcode/deleteQrCodeWeb", {
            userId: userId,
...
function editQrcode(qrcodeId) {
    var type = $("#select-qrcode-type-edit").val();
    var content = $("#et-note-edit").val();
    var title = $("#et-qrcode-name-edit").val();
    var userId = localStorage.getItem("userId");
 ...
 */
function addQrcode() {
    var type = $("#select-qrcode-type").val();
    var content = $("#et-note").val();
    var title = $("#et-qrcode-name").val();
    var userId = localStorage.getItem("userId");
